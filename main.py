...
def deposite():
    while True:
        amount = input("What is the amount you would like to deposite? $ ")
        if amount.isdigit():
            amount = int(amount)
            if amount > 0:
                break
...
def get_bet():
    while True:
        amount = input("What would you like to bet amount? $ ")
        if amount.isdigit():
            amount = int(amount)
...
def spin(balance):
    ...
    winnings, winning_lines = check_winnings(slot, bet, lines, symbols_value)
